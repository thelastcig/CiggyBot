...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, ":microphone2: **Join a voice channel to use this command**");
        if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":microphone: **Join the same voice channel as me to use this command!**");
        let SearchString = args.join(" ");
        if (!SearchString) return client.sendTime(message.channel, `**Usage - **\`${GuildDB.prefix}play [song]\``);
        let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
        let Searching = await message.channel.send(":drum: Looking for your song...");
        if (!CheckNode || !CheckNode.connected) {
       return client.sendTime(message.channel,":no_entry: **Lavalink node disconnected**");
        }
        ...
        if (!player) return client.sendTime(message.channel, ":dvd: **Nothing is playing right now, add some songs to get started**");
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Playlist Enqueued`, message.author.displayAvatarURL());
                    ...
                    SongAddedEmbed.setAuthor(`Enqueued`, client.botconfig.IconURL);
                    ...
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Queue Position", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                } else {
                    return client.sendTime(message.channel, "**:eyes: can't find anything for - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, ":dvd: **Nothing is playing right now, add some songs to get started**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**:eyes: can't find anything for - **" + SearchString);
                else if (Searched.loadType == "PLAYLIST_LOADED") {
                    player.queue.add(Searched.tracks);
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Playlist Enqueued`, client.botconfig.IconURL);
                    ...
                    SongAddedEmbed.addField("Artist", Searched.tracks[0].author, true);
                    SongAddedEmbed.addField("Track Length", `\`${prettyMilliseconds(Searched.tracks[0].duration, { colonNotation: true })}\``, true);
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Queue Position", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                }
            }
    ...
    SlashCommand: {
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!member.voice.channel) return client.sendTime(interaction, ":microphone2: **Join a voice channel to use this command**");
            if (guild.me.voice.channel && !guild.me.voice.channel.equals(member.voice.channel)) return client.sendTime(interaction, ":microphone: **Join the same voice channel as me to use this command!**");
            let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
            if (!CheckNode || !CheckNode.connected) {
              return client.sendTime(interaction,":no_entry: **Lavalink node disconnected**");
            }
